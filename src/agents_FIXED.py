...
def _chat_with_retry(client, **kwargs):
    """Call the chat API, retrying on rate-limit (429) with linear backoff."""
    for attempt in range(8):
        try:
            ...
            if "rate_limit" in msg or "429" in msg:
                wait = 8 * (attempt + 1)
                print(f"    rate limit hit - waiting {wait}s (attempt {attempt+1}/8)")
                time.sleep(wait)
                continue
            raise
    raise RuntimeError("rate limit retries exhausted after 8 attempts")
...
def agent3_plan(client, diagnosis, rul):
    ...
    resp = _chat_with_retry(
        client, model=OPENAI_MODEL, temperature=TEMPERATURE,
